...
env = GatheringEnv(2,"default_small2")  # gym.make('CartPole-v1')
env.seed(args.seed)
torch.manual_seed(args.seed)

# agentParam =

model_name = "gathering_1_default_"#"gathering_cent_"#gathering_1"
...
def main():
    ...
    #multiPG = CenAgents([Centralised_AC(8,state_dim,add_para(i)) for i in range(n_agents)],state_dim,agentParam)  # create PGagents as well as a social agent

    multiPG = Agents([IAC(8,400,add_para(i),useLaw=False) for i in range(n_agents)])  # create PGagents as well as a social agent
